...
import com.kynsof.identity.application.command.user.createPatientUser.CreatePatientUserCommand;
import com.kynsof.identity.application.command.user.createPatientUser.CreatePatientUserMessage;
import com.kynsof.identity.application.command.user.createPatientUser.CreatePatientUserRequest;
...
public class UserSystemController {
    ...
    /**
     * Endpoint especifico para crear usuarios de tipo paciente.
     * El ID del user_system sera igual al keycloakId generado por Keycloak.
     */
    @PostMapping("/patient")
    public ResponseEntity<?> createPatientUser(@RequestBody CreatePatientUserRequest request) {
        CreatePatientUserCommand command = CreatePatientUserCommand.fromRequest(request);
        CreatePatientUserMessage response = mediator.send(command);
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<?> me(
        ...
        try {
            ...
            UserMeQuery query = businessId != null
                    ? new UserMeQuery(UUID.fromString(userId), businessId)
                    : new UserMeQuery(UUID.fromString(userId));
